...
/** A complete, independent video router instance. */
export interface VideoRouter {
  videoState: import('svelte/store').Writable<VideoState>;
  videoStream: import('svelte/store').Writable<MediaStream | null>;
  videoRtcStats: import('svelte/store').Writable<VideoRtcStats | null>;
...
export function createVideoRouter(instanceId: 'video1' | 'video2') {
// ── Persistence ─────────────────────────────────────────────────────
...
function startActive(): Promise<void> {
...
/** Publish the second video widget's on-screen rect (map overlay in `widget2` mode). */
...
async function initVideo(): Promise<void> {
  ...
  // MJPEG sink handlers + subscription. The canvas sink is a single global
  // resource, so only the primary instance (video1) owns it. video2 renders
  // camera / RTSP-WebRTC MediaStreams, which never touch the MJPEG path.
  if (instanceId === 'video1') {
...
/** Global capability probe — a browser/media-stack fact, not per-instance. */
export const pipSupported =
  typeof document !== 'undefined' && !!document.pictureInPictureEnabled;

// Backwards-compatible named exports (default to video1) so existing imports
